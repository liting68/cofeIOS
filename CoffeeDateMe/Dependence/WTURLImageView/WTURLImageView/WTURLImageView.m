...
- (UIImage *)getImageWithURL:(NSString *)imageURL {
    ...
    NSString *cacheKey = [[self class] sanitizeFileNameString:imageURL];
...
///////type:0 默认有加载中的照片   type:1 没有加载中的照片
- (void)setURL:(NSString *)url defaultImage:(NSString *)imageName type:(int)type {
    
    if (!url || [url isKindOfClass:[NSNull class]] || [AppUtil isNull:url]) {
        
        if (self.isNoActi) {
            
            self.image = TTImage(@"noActi.jpg");
        }else {
              self.image = TTImage(imageName);
        }
        ...
        if (type == 0) {
            ...
            placeHolderImage =  imageName;//[NSString stringWithFormat:@"",imageName];
        
        }else {
            
            placeHolderImage = imageName;
        }
        
        self.image = TTImage(placeHolderImage);
        
        [self setURL:[NSURL URLWithString:url] fillType:UIImageResizeFillTypeFillIn options:WTURLImageViewOptionTransitionCrossDissolve|WTURLImageViewOptionDontUseConnectionCache | WTURLImageViewOptionDontClearImageBeforeLoading  placeholderImage:TTImage(placeHolderImage) failedImage:TTImage(imageName) diskCacheTimeoutInterval:0];
    }
...
///////包含高斯模糊算法  type:0 默认有加载中的照片   type:1 没有加载中的照片
- (void)setURLWithBlurry:(NSString *)url defaultImage:(NSString *)imageName type:(int)type {
    ...
    if (!url || [url isKindOfClass:[NSNull class]] || [AppUtil isNull:url]) {
        ...
        if (type == 0) {
            ...
            placeHolderImage =  [NSString stringWithFormat:@"%@_loading",imageName];
        ...
        __weak WTURLImageView * tImageView = self;
        
        [self setURLRequest:imageRequest fillType:UIImageResizeFillTypeFillIn options:WTURLImageViewOptionTransitionCrossDissolve|WTURLImageViewOptionDontUseConnectionCache | WTURLImageViewOptionDontClearImageBeforeLoading  placeholderImage:TTImage(placeHolderImage) failedImage:TTImage(imageName) diskCacheTimeoutInterval:0 success:^(NSURLRequest *request, NSHTTPURLResponse *response, UIImage *image) {
            
             UIImage * tImage = [AppUtil blurryImage:image withBlurLevel:1.0];
            
             [tImageView wt_makeTransition: tImage effect:WTURLImageViewOptionTransitionCrossDissolve|WTURLImageViewOptionDontUseConnectionCache | WTURLImageViewOptionDontClearImageBeforeLoading];
...
- (void)addGestureRecognizers {
    ...
    longPressReg.minimumPressDuration = 1.0;
    [self addGestureRecognizer:longPressReg];
...
- (void)singleTap:(UIGestureRecognizer *)singleTap {
    ...
    if (singleTap.state == UIGestureRecognizerStateEnded) {
         ...
         if (_delegate && [_delegate respondsToSelector : @selector(URLImageViewDidClicked:)]) {
...
- (void)longClick:(UIGestureRecognizer *)gesReg {
    ...
    if (gesReg.state == UIGestureRecognizerStateBegan) {
    
        if (_delegate && [_delegate respondsToSelector : @selector(URLImageViewDidLongPressed:)]) {
            [_delegate URLImageViewDidLongPressed : self];
        }
...
- (void)doubleClick:(UIGestureRecognizer *)gesReg {
    ...
    if (gesReg.state == UIGestureRecognizerStateEnded) {
        if (_delegate && [_delegate respondsToSelector : @selector(URLImageViewDidDoubleClicked:)]) {
            [_delegate URLImageViewDidDoubleClicked : self];
        }
