...
- (void)sw_addUtilityButtonWithColor:(UIColor *)color title:(NSString *)title
{
    ...
    button.frame = CGRectMake(K_UIMAINSCREEN_WIDTH - 77, 0, 77, 94);
    [button setBackgroundImage:TTImage(@"cancle_collect") forState:UIControlStateNormal];
    [button.titleLabel setAdjustsFontSizeToFitWidth:YES];
    [self addObject:button];
}

- (void)sw_addUtilityButtonForDeleteWithColor:(UIColor *)color title:(NSString *)title
{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.backgroundColor = color;
    [button setTitle:title forState:UIControlStateNormal];
    [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
   // button.frame = CGRectMake(K_UIMAINSCREEN_WIDTH - 60, 0, 77, 94);
    //[button setBackgroundImage:TTImage(@"cancle_collect") forState:UIControlStateNormal];
    [button.titleLabel setAdjustsFontSizeToFitWidth:YES];
    [self addObject:button];
}
