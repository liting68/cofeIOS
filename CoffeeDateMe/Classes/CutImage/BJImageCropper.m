...
- (void)updateBounds {
  ...
  // 1/2   widh  10  height  20
    
    if(self.scaleX < self.scaleY) {
        
        width = (height * self.scaleX)/self.scaleY;
        
    }else if(self.scaleX > self.scaleY){
        
        height = self.scaleY * width / self.scaleX;
        
    }else {
       
        int selfMin = width > height?height:width;
        width = selfMin;
        height = selfMin;
...
- (id)initWithImage:(UIImage*)newImage andMaxSize:(CGSize)maxSize scaleX:(CGFloat)scaleX scaleY:(CGFloat)scaleY {
